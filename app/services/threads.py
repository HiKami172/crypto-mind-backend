...
class ThreadService:
    ...
    async def create(self, unit_of_work: IUnitOfWork, data: ThreadCreateRequest, user_id: int):
    ...
    async def list_messages(self, unit_of_work: IUnitOfWork, page: int, per_page: int, **filter_by) -> dict:
        async with unit_of_work:
    ...
    async def add_message(self, unit_of_work: IUnitOfWork, thread_id: UUID4, message):
        async with unit_of_work:
            await unit_of_work.messages.create(id=uuid4(), thread_id=thread_id, content=message["content"], role=message["role"])
